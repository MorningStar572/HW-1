...
-- 1;
select first_name 
from restStaff inner join restBill on waiter_no = staff_no
where cust_name = 'Tanya Singh';

-- 2;
select room_date
from restStaff inner join restRoom_management as RomAng on RomAng.headwaiter = staff_no
where staff_no = '010' and room_date like '1602%';
...
from restStaff inner join restBill on waiter_no = staff_no
where (bill_no = '00014' or '00017');

-- 6;
select first_name, surname 
from restStaff inner join restRoom_management as rester on rester.headwaiter = staff_no
where room_name like 'Blue' and room_date = '160312';
