...
INSERT INTO restBill VALUES
 ...
 from restBill join restStaff on waiter_no = staff_no
 where waiter_no in (select waiter_no
 from restStaff
 ...
 where waiter_no in (select waiter_no
 from restStaff
 where first_name = "Sam") and bill_total > 400;
 ...
 as select room_name, bill_total total_sum
 from restRest_table join restBill;
 ...
 as select first_name headwaiter_name, bill_total total_sum 
 from restStaff join restBill
 group by headwaiter 
;
